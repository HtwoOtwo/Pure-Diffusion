...
class DiscreteGaussianSchedule(DiffusionSchedule):
    ...
    DiscreteGaussianSchedule manages all timestep properties that are a function of the variance schedule (betas). For example, to
    ...
    def __init__(self, betas: Tensor, variance_range_value: float = 0.0):
        ...
        if not 0.0 <= variance_range_value <= 1.0:
            raise ValueError("Variance_range_value must be between 0 and 1")
        self._betas = betas
        ...
        self.add_property("sqrt_compliment_alphas_cumprod", _sqrt_compliment_alphas_cumprod)
    ...
    def q_posterior(
        ...
        log_variance = variance_range_value * max_log + (1 - variance_range_value) * min_log
        mean = (
    ...
    def __call__(self, var_name: str, t: Tensor, shape: Union[Tensor, torch.Size]) -> Tensor:
...
# **************************************** Scheduler Functions ****************************************
def _alphas(schedule: DiscreteGaussianSchedule) -> Tensor:
    return 1.0 - schedule.betas


def _alphas_cumprod(schedule: DiscreteGaussianSchedule) -> Tensor:
    return schedule.alphas.cumprod(axis=0)


def _alphas_cumprod_prev(schedule: DiscreteGaussianSchedule) -> Tensor:
    return F.pad(schedule.alphas_cumprod[:-1], (1, 0), value=1.0)


def _sqrt_alphas_cumprod(schedule: DiscreteGaussianSchedule) -> Tensor:
...
def _upper_posterior_log_variance(schedule: DiscreteGaussianSchedule) -> Tensor:
    return schedule.betas.log()


def _posterior_mean_x0_coef(schedule: DiscreteGaussianSchedule) -> Tensor:
...
def _posterior_mean_xt_coef(schedule: DiscreteGaussianSchedule) -> Tensor:
...
# **************************************** Util functions ****************************************
def linear_interpolate(start_value: Tensor, end_value: Tensor, weight: Tensor) -> Tensor:
    """
